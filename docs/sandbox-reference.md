...
not cross-contaminate one another. Metadata probes of tracked-process procfs
magic links are mediated as well: `lstat`/`statx(..., AT_SYMLINK_NOFOLLOW)`
describe `/proc/<pid>/{exe,fd/*,cwd,root}` as links, while following `exe` or an
...
| `max_processes`  | `processes`   | `int`                   | `64`    | Maximum number of **concurrent** processes in the sandbox (peak, not lifetime; threads do not count). Also enables fork interception used by checkpoint freeze. A rejected fork returns `EAGAIN`; Sandlock writes a rate-limited `execution_domain_diagnostic` JSON object to the supervisor's stderr. It distinguishes Sandlock quota denial from a kernel fork failure and reports current/peak/limit counts, live reservations, tracked tasks, active creation traces and exec freezes, process-group membership, PTYs, supervisor FDs/threads, and Host process count. Process-creation tracking uses a bounded, thread-directed, unblockable watchdog wake for failed forks; if even bounded teardown cannot finish, only the captured execution process group is killed after its stable task identity is revalidated. Exec argv freeze canonicalizes indexed TIDs to TGIDs before walking threads and waits boundedly for another tracker thread in the same supervisor to hand off, so multithreaded runtimes are neither seized twice nor rejected during a fork-to-exec overlap. Freeze setup/cleanup failures use the same structured log prefix and include each tracked task's state and `TracerPid`. |
